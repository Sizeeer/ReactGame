...
const BiographyElement = ({ el }) => {
  if (el.type === "h1") {
    return <Heading black>{el.text}</Heading>;
  }
  if (el.type === "h2") {
    return <Heading level={2}>{el.text}</Heading>;
  }

  if (el.type === "paragraph") {
    return <Text element="p">{el.text}</Text>;
  }

  return <img src={el.src} alt="character avatar" />;
};

BiographyElement.propTypes = {
  el: PropTypes.shape({
    type: PropTypes.oneOf(["h1", "h2", "paragraph", "img"]).isRequired,
    ...
    <section className={styles.biographySection}>
      <Container className={styles.container}>
        <div className={styles.backButtonWrapper}>
          <Button isGoBack onClick={handleBackClick} />
        </div>
        {BIO[id].map((el, i) => (
